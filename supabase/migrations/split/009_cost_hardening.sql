-- =============================================================================
-- 009 — Cost Versioning: Hardening (aplicar SOLO si se usó 008 monolítico)
-- =============================================================================
-- QUÉ HACE: parcha las funciones y constraints de 008 monolítico.
--   FIX 1: get_resource_cost + get_resource_snapshot con guards de aislamiento
--   FIX 2: generate_monthly_snapshots con p_overwrite semántico correcto
--   FIX 3: constraint endurecido en resource_pricing_rules
--   FIX 4: trigger sync_current_snapshot_cache (reemplaza UPDATE manual)
--   FIX 5: upsert_index_value con autorización explícita
--
-- PREREQUISITOS: 008_cost_versioning.sql (monolítico) ejecutado.
--
-- ⚠ OMITIR si ejecutaste la serie split (008a → 008b → 008c).
--   En ese caso las versiones finales ya están en 008c.
--
-- IDEMPOTENTE: sí (CREATE OR REPLACE, DROP TRIGGER IF EXISTS, ALTER + DROP + ADD CONSTRAINT).
--
-- VERIFICAR DESPUÉS:
--   SELECT pg_get_functiondef(oid)
--   FROM pg_proc WHERE proname = 'get_resource_cost' AND pronamespace = 'public'::regnamespace;
--   -- El body debe incluir "auth.uid() IS NOT NULL AND NOT is_org_member".
-- =============================================================================

BEGIN;

-- =============================================================================
-- FIX 3 — Helpers para constraints IMMUTABLE
-- =============================================================================
...
-- =============================================================================
-- FIX 4 — Trigger sync_current_snapshot_cache
-- =============================================================================
...
-- =============================================================================
-- FIX 1 — get_resource_cost con guards de aislamiento
-- =============================================================================
...
-- =============================================================================
-- FIX 1 — get_resource_snapshot con guards de aislamiento
-- =============================================================================
...
-- p_overwrite=true  → ON CONFLICT DO UPDATE
-- p_overwrite=false → ON CONFLICT DO NOTHING
-- =============================================================================
...
DECLARE
  ...
  v_i                int;
BEGIN
  -- service_role bypass (auth.uid() IS NULL)
  ...
  LOOP
    v_cost := NULL; v_status := 'error'; v_src := NULL;

    IF NOT p_overwrite AND EXISTS (
      SELECT 1 FROM public.resource_cost_snapshots
      WHERE tenant_id = p_tenant_id AND resource_id = v_rule.resource_id
        AND effective_date = v_eff_date AND source_type <> 'FALLBACK_BASE_COST'
    ) THEN
      resource_id := v_rule.resource_id; resource_code := v_rule.res_code;
      rule_type := v_rule.rule_type; cost := NULL; status := 'skipped'; RETURN NEXT; CONTINUE;
    END IF;
    ...
    ELSIF v_rule.rule_type = 'COMPOSITE_INDEX' THEN
      v_comps := v_rule.formula_config -> 'components'; v_weighted_sum := 0;
    ...
    resource_id := v_rule.resource_id; resource_code := v_rule.res_code;
    rule_type := v_rule.rule_type; cost := v_cost; status := v_status;
    RETURN NEXT;
  END LOOP;
END;
$$;
